...
const SHARED_RULES = `
【类型判断规则（严格执行）】
- vocabulary: 单个词语，包括单个名词、动词、形容词、副词、汉字词。判断标准：能单独查到词典的独立词汇。
- grammar: N1语法句型，通常含助词组合或接续形式（如：〜にもかかわらず、〜を皮切りに、〜に際して、〜ずにはおかない、〜に相違ない）
- collocation: 由两个以上词组成的惯用表达/惯用句，整体才有特殊意义（如：気が置けない、手が込む、一石二鳥、腹を割る）。判断标准：拆开每个词后整体含义消失。

【提取要求】
1. 提取所有N1词汇和语法，包括题目正文和全部选项（选择题4个选项都要检查）
2. 所有N1语法句型必须提取（type=grammar），哪怕只出现一次，绝对不能遗漏
3. 语法的term字段写完整句型（如：〜にもかかわらず），connection字段写接续方式（如：名词/普通形+にもかかわらず）
4. 词汇若在文中以变形出现（如动词活用、形容词变形），term字段必须写辞书形/原形（如：見かねた→見かねる、悔しくて→悔しい）
5. 词汇的reading字段填假名读音（辞书形的读音）
6. collocation和vocabulary不要搞混：単独の词→vocabulary，多词惯用→collocation
7. 不要提取「文法」「語彙」「読解」「聴解」「問題」等章节标题
8. 提取完毕后，回顾一遍是否有遗漏的语法点或词汇，再输出结果`
